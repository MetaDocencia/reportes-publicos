...
Over the last 5 years, MetaDoncia has taught `r t1[1,2]` editions and a total of `r t1[1,3]` hours of `r t1[1,1]` different workshops. We reached `r t1[1,4]` educators, researchers and technicians, of which `r round(t1[1,5]/t1[1,4]*100,1)`% attended synchronously and `r t1b[1,1]`% completed the post workshop survey, in which we obtained an average Net Promoter Score (NPS) of `r t1[1,8]`% (min = `r min(ws$NPS, na.rm = TRUE)`, max = `r max(ws$NPS, na.rm = TRUE)`). This is considered excellent, as the NPS ranges from -100% to +100%, and values larger than +50%—our target—reflect very high satisfaction from the participants.
